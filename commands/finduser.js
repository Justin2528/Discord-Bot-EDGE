...
const tools = require('../function'); // This is the primary package we will be using.
...
exports.run = (client, message, args, tools) => { // This is the same handler we have been using in the previous episodes.
 ...
 let embed = new Discord.RichEmbed()
  ...
  if (!args[0]) return embed.setFooter('Please input some text').then(message.channel.send(embed)); 
  // This is from the `Global Functions` episode. - This checks to see if args[0] is defined, if it's not it runs the return statement.
  ...
  embed.setDescription('Do you mean: ' + member).then(message.channel.send(embed)); // Now, we can post in chat what the autocomplete is.
